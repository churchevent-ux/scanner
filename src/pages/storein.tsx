...
export default function StoreInPage({ setPath }: any) {
  ...
  const onEnterValue = async (value: string) => {
    ...
    if (value.length > 4) {
      ...
      if (!user) {
        ...
        showWarningToast("User already stored something");
      } else {
        ...
        } else showErrorToast("Failed On Store In");
      }
      setuserId("");
    }
    setbusy(false);
