...
export default function SignOutPage({ setPath }: any) {
  ...
  const loadData = async (_page: number) => {
    // setpage(_page);
    let res: any = await getAllUsersSignedOut(_page);
    // setusers(res.data || []);
  ...
  const onEnterValue = async (value: string) => {
    ...
    if (value.length > 4) {
      ...
      if (!user) {
        ...
        showWarningToast("User have something in the store");
      } else {
        ...
        } else showErrorToast("Failed On Sign Out");
      }
      setuserId("");
    }
    setbusy(false);
