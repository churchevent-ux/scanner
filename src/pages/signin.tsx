...
export default function SignInPage({ setPath }: any) {
  ...
  const onEnterValue = async (value: string) => {
    if (value.length > 4) {
      ...
      if (!user) {
        ...
        showWarningToast("User already signed In");
      } else {
        ...
        <div className="inline-flex items-center gap-3 bg-white/60 px-4 py-2 rounded-full shadow-sm">
          ...
          <div className="text-left">
            ...
            <p className="mt-1 text-xs lg:text-sm text-gray-500">
              Scan a QR code or type your ID to sign in quickly and securely.
            </p>
          </div>
        </div>
      </div>
